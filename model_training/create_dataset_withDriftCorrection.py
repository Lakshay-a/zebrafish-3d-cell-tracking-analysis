...
# ============================================================
# FINAL DISSERTATION DATASET SETTINGS
# ============================================================
...
# Use first channel only: green/GFP/muSC channel.
...
DEFAULT_IGNORE_LAST_Z = 1

# Datasets where early Z-slices often contain skin/surface.
...
# Exclude bad acquisition blocks.
...
def estimate_drift_all_timepoints_option_b(reader):
    """
...
def get_shift_for_timepoint(time_indices, shifts_yx, t_idx):
    ...
    if int(t_idx) not in time_to_i:
        raise ValueError(
            ...
            f"Use DRIFT_TIMEPOINT_STEP=1 for final dataset creation."
        )
...
def save_corrected_training_slice(
    ...
    return {
        ...
        "channel_index": GREEN_CHANNEL_INDEX,
...
def process_one_czi(czi_path, split, file_index, save_drift_qc_for_this_file=False):
    ...
    if reader.size_c <= GREEN_CHANNEL_INDEX:
        raise ValueError(
            f"GREEN_CHANNEL_INDEX={GREEN_CHANNEL_INDEX} requested, "
    ...
    print("[DRIFT] Estimating final Option-B drift across ALL timepoints...")
...
def main():
    ...
    print("\n[STEP 1] Finding CZI/acquisition-block files...")
    czi_files = find_czi_files()

    if len(czi_files) == 0:
        raise RuntimeError("No CZI files found. Check SOURCE_PATHS.")

    print(f"[INFO] Found {len(czi_files)} CZI files after exclusions.")

    print("\n[STEP 2] Creating CZI inventory...")
    inventory_df = create_inventory(czi_files)
    ...
    if len(usable_paths) == 0:
        raise RuntimeError("No readable CZI files found after inventory.")

    print(f"[INFO] Readable CZI files: {len(usable_paths)}")

    print("\n[STEP 3] Creating CZI/acquisition-block level train/val/test split...")
    split_df = split_files(usable_paths)

    print("\n[INFO] Split counts:")
    print(split_df["split"].value_counts())

    print("\n[STEP 4] Running final drift correction and extracting corrected slices...")
    ...
    if len(processing_errors) > 0:
        pd.DataFrame(processing_errors).to_csv(
    ...
    print(f"Readable CZI/acquisition-block files: {len(usable_paths)}")
    print(f"Extracted candidate slices: {len(extracted_df)}")
    ...
    if len(extracted_df) > 0:
        ...
        print("\nSlices per time region:")
        print(extracted_df["time_region"].value_counts())

    if len(processing_errors) > 0:
        print("\n[WARNING] Some files failed. See:")
        print(METADATA_DIR / "processing_errors.csv")

    print("\nFor drift evaluation/report, inspect:")
    print(DRIFT_QC_DIR)
